document.addEventListener("DOMContentLoaded", function (event) {
    ...
    // Experiences
    document.querySelector(".project-section .companies-list").addEventListener('click', function (e) {
        e.preventDefault();
        if (e.target.tagName === 'LI') {
           window.innerWidth > 992 ? document.querySelector(".project-section .selector").style.top = e.target.offsetTop + 'px' : null;
            document.querySelector(".project-section .companies-list li.active").classList.remove('active')
            e.target.classList.add('active');
            var targetTab = e.target.getAttribute('data-tab');
            if (targetTab) {
                document.querySelector(".project-section .content.active").classList.remove('active')
